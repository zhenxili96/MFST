...
def main(model_config, train_config, track_config):
  ...
  with g.as_default():
    ...
    opt_op_c5 = tf.contrib.layers.optimize_loss(
      loss=model.batch_loss_c5,
    ...
    opt_op_c4 = tf.contrib.layers.optimize_loss(
      loss=model.batch_loss_c4,
      global_step=model.global_step,
      learning_rate=learning_rate,
      optimizer=optimizer,
      clip_gradients=train_config['clip_gradients'],
      learning_rate_decay_fn=None,
      summaries=['learning_rate'])
    opt_op_c3 = tf.contrib.layers.optimize_loss(
      loss=model.batch_loss_c3,
      global_step=model.global_step,
      learning_rate=learning_rate,
      optimizer=optimizer,
      clip_gradients=train_config['clip_gradients'],
    ...
    with tf.control_dependencies([opt_op_c5, opt_op_c4, opt_op_c3]):
...
# =============================================================================
    ...
    if not model_path:
      ...
      sess.run(global_variables_init_op)   ## for the global step var
      sess.run(local_variables_init_op)      
      ...
      for key, value in variables_to_restore.items():
          if key.split('/')[1] != 'State':
              if 'OptimizeLoss' in key:
                  continue
              if 'global_step' in key:
                  continue
              if 'detection/weight_s' in key:
                  continue
              if 'detection/biases_s' in key:
                  continue
              if '_ex_' in key:
                  continue
              if '_sq_' in key:
      ...
      _saver = tf.train.Saver(variables_to_restore_filterd)
...
# =============================================================================
    ...
    for step in range(start_step, total_steps):
      ...
      _, loss, batch_loss_c5, batch_loss_c4, batch_loss_c3 = sess.run([train_op, 
                                               model.total_loss, model.batch_loss_c5,
                                               model.batch_loss_c4, model.batch_loss_c3])
      ...
      if step % 40 == 0:
        ...
        h, m = divmod(m, 60)
        format_str = ('%s: step %d, total loss = %.2f, batch loss c5 = %.2f batch loss c4 = %.2f batch loss c3 = %.2f '
                      '(%.1f examples/sec; %.3f '
                      'sec/batch; %dh:%02dm:%02ds remains)')
        logging.info(format_str % (datetime.now(), step, loss, batch_loss_c5, batch_loss_c4, batch_loss_c3,
